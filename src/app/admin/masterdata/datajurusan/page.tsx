...
export default function DataJurusan() {
  ...
  useEffect(() => {
    if (tahun) {
      // Fetch data jurusan from API or other sources
      axios.get(`http://localhost:2008/admin/jurusan?id_tahun=${tahun}&page=1`,{ withCredentials: true }).then((res) => {
  ...
  return (
    <div>
      <Banner title="Data jurusan" />
      <Link
        href="/admin/masterdata/datajurusan/tambahdata"
          ...
          <table className="min-w-full divide-y divide-black overflow-scroll">
            <thead>
              <tr className="divide-x-2 divide-black text-sm">
                ...
                <th className="py-2 text-center font-medium">Nama Jurusan</th>
                <th className="py-2 font-medium">Act</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-black">
              {jurusan.map((data:any, index) => (
                <tr key={data.id} className="divide-x-2 divide-black">
                  ...
                  <td className="text-end">
                      <ActionButton tipe="jurusan" id={data.id} />
                  </td>
                </tr>
              ))}
            </tbody>
