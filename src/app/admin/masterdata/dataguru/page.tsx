...
export default function DataGuru() {
  ...
  useEffect(() => {
    if (tahun) {
      // Fetch data guru from API or other sources
      axios.get(`http://localhost:2008/admin/guru-pembimbing?id_tahun=${tahun}&page=1`,{ withCredentials: true }).then((res) => {
  ...
  return (
    <div>
      <Banner title="Data guru" />
      <Link
        href="/admin/masterdata/dataguru/tambahdata"
          ...
          <table className="min-w-full divide-y divide-black overflow-scroll">
            <thead>
              <tr className="divide-x-2 divide-black text-sm">
                ...
                <th className="py-2 text-center font-medium">Nama Guru</th>
                <th className="px-4 py-2 text-center font-medium">NIP</th>
                <th className="px-2 py-2 font-medium text-nowrap">Jenis Kelamin</th>
                <th className="px-2 py-2 font-medium">No Telepon</th>
                <th className="py-2 px-2 font-medium">Alamat</th>
                <th className="py-2 px-2 font-medium">Tanggal Lahir</th>
                <th className="py-2 font-medium">Act</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-black">
              {guru.map((data:any, index) => (
                <tr key={data.id} className="divide-x-2 divide-black">
                  ...
                  <td className="px-4 py-1">{data.nip}</td>
                  ...
                  <td className="px-4 text-nowrap py-1">{data.tanggal_lahir}</td>
                  <td className="text-end">
                      <ActionButton tipe="guru" id={data.id} />
                  </td>
                </tr>
              ))}
            </tbody>
